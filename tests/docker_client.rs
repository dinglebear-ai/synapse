...
//! remote docker host is provided via the `SYNAPSE_TEST_REMOTE_HOST` env var
...
/// `SYNAPSE_TEST_REMOTE_HOST` names a reachable SSH host (from `~/.ssh/config`)
...
async fn remote_forwarded_socket_same_shape() {
    let Ok(host_name) = std::env::var("SYNAPSE_TEST_REMOTE_HOST") else {
        eprintln!("skipping: set SYNAPSE_TEST_REMOTE_HOST to a docker-running SSH host");
        return;
    };
    ...
    let host = HostConfig {
        ...
        host: host_name,
        protocol: synapse::synapse::HostProtocol::Ssh,
        ..HostConfig::local()
    };
