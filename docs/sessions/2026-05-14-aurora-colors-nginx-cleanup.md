...
15. Ran `tailscale status` to identify devices: devhost=198.51.100.1, edgehost=198.51.100.2, nashost=198.51.100.3
...
- **apprise MCP upstream on devhost, not nashost**: The apprise web service runs on nashost (198.51.100.3:8766), but apprise-mcp (the Rust sidecar) runs on devhost (198.51.100.1:40050). Split upstream/mcp_upstream accordingly.
...
- **Created** `rmcp-example.subdomain.conf` — `rmcp.example.internal`, `198.51.100.1:40060`, authelia on `/`
...
| rmcp.example.internal | Did not exist | New conf; rmcp-example at `198.51.100.1:40060`; authelia on `/` |
...
- **apprise-mcp port**: The apprise-mcp sidecar (`198.51.100.1:40050`) must be running for MCP calls to work. The upstream Apprise web UI (`198.51.100.3:8766`) is unaffected.
