//! `synapse watch` — health monitor for the MCP HTTP server.
//!
...
pub async fn run_watch(base_url: &str, interval_secs: u64) -> Result<()> {
    ...
    eprintln!(
        "[synapse watch] polling {} every {}s — emitting stdout on state change",
    ...
    match client.get(url).send().await {
        ...
        Err(e) => {
            ...
            if !e.is_timeout() && !e.is_connect() {
                eprintln!("[synapse watch] probe error (may indicate config issue): {e}");
            }
            ServerState::Down
        }
    }
...
fn format_event(
    ...
    match (current, prev) {
        ...
        (ServerState::Up, Some(prev_state @ (ServerState::Down | ServerState::Degraded(_)))) => {
            format!(
                "[synapse] UP — {} recovered after {}s (was {})",
                base_url,
        ...
        (ServerState::Up, _) => format!("[synapse] UP — {} is healthy", base_url),
        // Went down
        (ServerState::Down, _) => format!(
            "[synapse] DOWN — {} is unreachable (retrying every {}s)",
            base_url, interval_secs
        ),
        // Degraded (non-2xx)
        (ServerState::Degraded(code), _) => {
            format!("[synapse] DEGRADED — {} returned HTTP {code}", base_url)
        }
    }
}
