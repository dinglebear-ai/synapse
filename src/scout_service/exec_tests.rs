...
impl SshExecutor for AlwaysOkExec {
    ...
    async fn transfer_file(&self, _: &HostConfig, _: &str, _: &HostConfig, _: &str) -> Result<u64> {
        Ok(2)
    }
}
...
#[tokio::test]
async fn beam_rejects_sensitive_source_before_transfer() {
    let mut host = HostConfig::local();
    host.scout_read_roots = vec!["/tmp".into()];
    let result = super::beam(
        &host,
        "/tmp/.ssh/id_ed25519",
        &host,
        "/tmp/dest",
...
#[tokio::test]
async fn beam_rejects_source_outside_configured_root() {
    let mut host = HostConfig::local();
    host.scout_read_roots = vec!["/tmp/allowed".into()];
    let result = super::beam(
        &host,
        "/tmp/outside",
        &host,
    ...
    assert!(
        result
            .unwrap_err()
            .to_string()
            .contains("outside configured")
    );
}

#[tokio::test]
async fn beam_copies_local_file_through_descriptor_bound_executor() {
    let dir = tempfile::tempdir().unwrap();
    let source = dir.path().join("source.txt");
    let destination = dir.path().join("destination.txt");
    std::fs::write(&source, b"beam payload").unwrap();

    let mut host = HostConfig::local();
    host.scout_read_roots = vec![dir.path().to_string_lossy().into_owned()];
    let pool = crate::ssh::SshPool::new();
    let result = super::beam(
        &host,
    ...
    assert_eq!(std::fs::read(destination).unwrap(), b"beam payload");
}
