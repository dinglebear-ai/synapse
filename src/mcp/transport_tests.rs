...
fn allowed_hosts_includes_bound_host_and_port_variant() {
    let hosts = allowed_hosts(&config("myhost.synapse.com", 8080));
    assert!(hosts.contains(&"myhost.synapse.com".to_string()));
    assert!(hosts.contains(&"myhost.synapse.com:8080".to_string()));
}
...
fn allowed_origins_includes_extra_allowed_origins() {
    let cfg = McpConfig {
        host: "0.0.0.0".to_string(),
        port: 3000,
        allowed_origins: vec!["https://app.synapse.com".to_string()],
        ..Default::default()
    };
    let origins = allowed_origins(&cfg);
    assert!(origins.contains(&"https://app.synapse.com".to_string()));
}
...
fn allowed_origins_normalizes_extra_allowed_origins() {
    let cfg = McpConfig {
        host: "0.0.0.0".to_string(),
        port: 3000,
        allowed_origins: vec!["https://app.synapse.com/some/path?ignored=true".to_string()],
        ..Default::default()
    };
    let origins = allowed_origins(&cfg);
    assert!(origins.contains(&"https://app.synapse.com".to_string()));
    assert!(!origins.contains(&"https://app.synapse.com/some/path?ignored=true".to_string()));
}
...
fn allowed_origins_skips_invalid_and_wildcard_origins() {
    let cfg = McpConfig {
        host: "0.0.0.0".to_string(),
        port: 3000,
        allowed_origins: vec!["not-a-url".to_string(), "https://*.synapse.com".to_string()],
        ..Default::default()
    };
    let origins = allowed_origins(&cfg);
    assert!(!origins.contains(&"not-a-url".to_string()));
    assert!(!origins.contains(&"https://*.synapse.com".to_string()));
}
...
fn allowed_origins_preserves_non_http_configured_origins() {
    let cfg = McpConfig {
        host: "0.0.0.0".to_string(),
        port: 3000,
        allowed_origins: vec!["vscode-webview://extension.synapse".to_string()],
        ..Default::default()
    };
    let origins = allowed_origins(&cfg);
    assert!(origins.contains(&"vscode-webview://extension.synapse".to_string()));
}
...
fn allowed_origins_includes_public_url_origin() {
    let cfg = McpConfig {
        ...
        port: 3000,
        auth: AuthConfig {
            public_url: Some("https://mcp.synapse.com".to_string()),
            ..Default::default()
        },
        ..Default::default()
    };
    let origins = allowed_origins(&cfg);
    assert!(origins.contains(&"https://mcp.synapse.com".to_string()));
}
